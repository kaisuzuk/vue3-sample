...
開発中に正常系・異常系を簡単に切り替えられるUIが用意されています。
...
| シナリオ | 説明 | 用途 |
|----------|------|------|
| 正常系（デフォルト） | 通常データ | 一般的な開発 |
| 空データ（0件） | 空配列を返す | 空表示の確認 |
| 3秒遅延 | レスポンスを遅延 | ローディング表示の確認 |
| サーバーエラー（500） | 500を返す | エラーハンドリングの確認 |
| バリデーションエラー（400） | 400を返す | フォームエラーの確認 |
| ネットワークエラー | 通信失敗 | オフライン対応の確認 |

### シナリオの永続化

選択したシナリオはローカルストレージに保存されます。
ブラウザを閉じても次回アクセス時に前回のシナリオが維持されます。

「正常系」に戻すには、UIで「正常系（デフォルト）」を選択してください。
